...
a1=data_test.predict_category_property.map(lambda x:x.split(';'))
b1=a1.apply(lambda x:len(x))
...
def data_origin(data) :
    ...
    data['item_property_list_num']=b1.apply(lambda x:len(x))
    ...
    data['predict_category_property_num']=c1.apply(lambda x :len(x))
...
#############查看test的数据是否包含在train中
aa=data_train1['user_id'].unique()
bb=data_test1['user_id'].unique()
cc=list(set(bb).difference(set(aa)))######bb有而aa没有
########统计每列数据中数据重复出现的个数
...
data_set1=data1[(data1.day>=18)&(data1.day<=23)].reset_index()
data_set2=data1[(data1.day>=19)&(data1.day<=24)].reset_index()
data_set3=data1[(data1.day>=20)&(data1.day<=25)].reset_index()
...
#all_item_data.to_csv('all_item_data1.csv',index=None)
...
#######user and gender 交叉
u1=data[['user_id','user_gender_id']]
u1['user_gender_saw']=1
u1=u1.groupby(['user_id','user_gender_id']).agg('sum').reset_index()
user_gender_saw=u1

#####user and age 奇偶差
u1=data[['user_id','user_age_level']]
u1['user_age_saw']=1
u1=u1.groupby(['user_id','user_age_level']).agg('sum').reset_index()
user_age_saw=u1
######user and occu 交叉
u1=data[['user_id','user_occupation_id']]
u1['user_occu_saw']=1
u1=u1.groupby(['user_id','user_occupation_id']).agg('sum').reset_index()
user_occu_saw=u1
...
all_user_data=pd.merge(all_user_data,user_gender_saw,on=['user_id','user_gender_id'],how='left')
all_user_data=pd.merge(all_user_data,user_age_saw,on=['user_id','user_age_level'],how='left')
all_user_data=pd.merge(all_user_data,user_occu_saw,on=['user_id','user_occupation_id'],how='left')


all_user_data.fillna(0)
#all_user_data.to_csv('all_user_data1.csv',index=None)
...
u1=data[['hour']]
u1['hour_see']=1
u1=u1.groupby('hour').agg('sum').reset_index()
hour_see=u1
######### houer  and pre_cate
u1=data[['hour','predict_category_property_m1']]
u1['hour_pre_cate']=1
u1=u1.groupby(['hour','predict_category_property_m1']).agg('sum').reset_index()
hour_pre_cate=u1
...
all_page_data=pd.merge(all_page_data,hour_see,on=['hour'],how='left')
all_page_data=pd.merge(all_page_data,hour_pre_cate,on=['hour','predict_category_property_m1'],how='left')


all_page_data.fillna(0)
#all_page_data.to_csv('all_page_data1.csv',index=None)
...
#all_shop_data.to_csv('all_shop_data1.csv',index=None)
...
item_cate_proper=u1
#####item_property m1 and pre_cate
u1=data[['item_property_list_m1','predict_category_property_m1']]
u1['item_pro_m1_pre']=1
u1=u1.groupby(['item_property_list_m1','predict_category_property_m1']).agg('sum').reset_index()
item_pro_m1_pre=u1

#####item_property m2 and pre_cate
u1=data[['item_property_list_m2','predict_category_property_m1']]
u1['item_pro_m2_pre']=1
u1=u1.groupby(['item_property_list_m2','predict_category_property_m1']).agg('sum').reset_index()
item_pro_m2_pre=u1
#####item_property m3 and pre_cate
u1=data[['item_property_list_m3','predict_category_property_m1']]
u1['item_pro_m3_pre']=1
u1=u1.groupby(['item_property_list_m3','predict_category_property_m1']).agg('sum').reset_index()
item_pro_m3_pre=u1

#########itrm_nrand and pre-cate
u1=data[['item_brand_id','predict_category_property_m1']]
u1['item_brand_precate']=1
u1=u1.groupby(['item_brand_id','predict_category_property_m1']).agg('sum').reset_index()
item_brand_precate=u1
...
all_two_data=pd.merge(all_two_data,item_pro_m1_pre,on=['item_property_list_m1','predict_category_property_m1'],how='left')
all_two_data=pd.merge(all_two_data,item_pro_m2_pre,on=['item_property_list_m2','predict_category_property_m1'],how='left')
all_two_data=pd.merge(all_two_data,item_pro_m3_pre,on=['item_property_list_m3','predict_category_property_m1'],how='left')
all_two_data=pd.merge(all_two_data,item_brand_precate,on=['item_brand_id','predict_category_property_m1'],how='left')
...
#all_two_data.to_csv('all_two_data1.csv',index=None)
...
#u1['is_new_user']=1
#u1.loc[u1.time_delta_user==-1,'is_new_user']=0
...
u1['is_new_user_brand']=1
u1.loc[u1.time_delta_user_brand==-1,'is_new_user_brand']=0
...
u1.drop_duplicates(inplace=True)
u1['user_shift']=u1['user_id'].shift(1)
u1['cate_shift']=u1['item_category_list'].shift(1)
u1['time_delta_user_item_cate']=u1['context_timestamp']-u1['context_timestamp'].shift(1)
u1.loc[(u1.user_shift !=u1.user_id)|(u1.cate_shift !=u1.item_category_list),'time_delta_user_item_cate']=-1
u1['is_new_user_item_cate']=1
u1.loc[u1.time_delta_user_item_cate==-1,'is_new_user_item_cate']=0
u1=u1.drop(['user_shift','cate_shift'],axis=1)
time_delta_user_item_cate=u1
...
u1['is_new_user_shop']=1
u1.loc[u1.time_delta_user_shop==-1,'is_new_user_shop']=0
...
shop_day_cumsum=u1.drop(['shop_id','day','shop_day_cumsum'],axis=1)
...
user_shop_day_cumsum=u1.drop(['user_id','shop_id','day','user_shop_day_times'],axis=1)
...
###################itrm category  and iten proper
u1=data[['item_category_list','item_property_list_m1','day']]
u1['item_cate_pro_times']=1
item_cate_pro_sum=u1.groupby(['item_category_list','item_property_list_m1','day']).agg('sum').reset_index()
u1['item_cate_pro_cumsum']=u1.groupby(['item_category_list','item_property_list_m1'])['item_cate_pro_times'].cumsum()
u1['item_cate_pro_day_cumsum']=u1.groupby(['item_category_list','item_property_list_m1','day'])['item_cate_pro_times'].cumsum()
item_cate_pro_day_cumsum=u1.drop(['item_category_list','item_property_list_m1','day','item_cate_pro_times'],axis=1)
...
all_time_data=pd.merge(all_time_data,time_delta_user_item_cate,on=['user_id','item_category_list','context_timestamp'],how='left')
...
all_time_data=pd.merge(all_time_data,item_cate_pro_sum,on=['item_category_list','item_property_list_m1','day'],how='left')
all_time_data=pd.concat([all_time_data,item_cate_pro_day_cumsum],axis=1,join='outer')



all_time_data.fillna(0)
#all_time_data.to_csv('all_time_data1.csv',index=None)
...
string11=new_data.columns.values.tolist()
print(string11)


new_data.to_csv('data_set1.csv',index=None)
...
train=new_train_data[(new_train_data.day<24)].drop(['day'],axis=1,inplace=False)
test=new_train_data[(new_train_data.day==24)].drop(['day'],axis=1,inplace=False)
...
#         min_child_samples=8,
        subsample=0.9,
        n_estimators=20000)
lgb_model = lgb0.fit(X_train, y_train, eval_set=[(X_test,y_test)],feature_name=string2,categorical_feature=string3,early_stopping_rounds=200)
