...
#####查看类别性变量
text_cols=[]
for ss in df_train.columns:
    if df_train[ss].dtype==np.object:
        text_cols.append(ss)
##########查看每一天的交易情况
...
new_data=pd.concat([new_data,all_item_data],axis=1)
del all_item_data
new_data=pd.concat([new_data,all_user_data],axis=1)
del all_user_data
new_data=pd.concat([new_data,all_page_data],axis=1)
del all_page_data
...
#lgb_model = lgb0.fit(X_train, y_train, eval_set=[(X_test,y_test)],feature_name=string2, categorical_feature=string3,eval_metric='binary_logloss',early_stopping_rounds=100)
...
joblib.dump(lgb_model,'fusai_7day_201852180455.pkl')
...
#        n_estimators=20000)
#
#lgb_model = lgb1.fit(X_train, y_train, eval_set=[(X_test,y_test)],eval_metric='binary_logloss',early_stopping_rounds=100)
...
dtrain1=xgb.DMatrix(train,y_train.is_trade)
dval=xgb.DMatrix(test, y_test.is_trade)
