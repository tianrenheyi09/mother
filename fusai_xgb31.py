...
import matplotlib.pyplot as plt
plt.figure(figsize=(8,6),dpi=120)
sns.countplot(y='hour',hue='is_trade',data=tr_data)


##########平滑的转化率

tmp=tr_data.groupby(['hour'])['is_trade'].agg({'count','sum'}).reset_index()
I=tmp['count']
C=tmp['sum']
hyper.update_from_data_by_FPI(I, C, 1000, 0.00000001)
print (hyper.beta,hyper.alpha)
tmp['cvr']=(C+hyper.alpha)/(I+hyper.beta+hyper.alpha)
#####
...
pd.DataFrame(pres_offline).to_csv('ensamble/pre_off_xgb_1705862.txt',index=False)
...
bao_cun.to_csv('round2_result/xgb0170586.txt',index=False,sep=' ')
