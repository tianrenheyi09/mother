# -*- coding: utf-8 -*-
"""
Created on Sat May 12 12:20:01 2018
...
data_day5=data1[data1.day !=7]
#data_day5=data1[(data1.day==5)|(data1.day==6)]
data_day6=data_day5[(data_day5.day==5)|(data_day5.day==6)]
data_day7=data1[data1.day==7]
data1=data1[data1.day==7]
data1=data_day7.copy()
###########计算第5天的点击率merge到第7天
...
for ss in string_id_cvr:
    ...
    I=tmp['count']
    C=tmp['sum']
    hyper.update_from_data_by_FPI(I, C, 1000, 0.0001)
#    print (hyper.beta,hyper.alpha)
    tmp[ss+'_cvr']=(C+hyper.alpha)/(I+hyper.beta+hyper.alpha)
#    tmp=tmp.drop(['count','sum'],axis=1)
    tmp.rename(columns={'count':ss+'_hiscount','sum':ss+'_hissum'},inplace=True)
    data1=pd.merge(data1,tmp,on=ss,how='left')
    

string_shop=['user_id','user_gender_id','user_occupation_id','user_age_level','item_id','item_category_list','item_brand_id','item_city_id', 'item_property_list_m1', 'item_property_list_m2','item_property_list_m3']

#u1=data_day5.groupby(['shop_id','user_age_level'])['is_trade'].count().reset_index()
#u1.rename(columns={'is_trade':'his_shop_'+'user_age_level'},inplace=True)


for ss in string_shop:
    u1=data_day5.groupby(['shop_id',ss])['is_trade'].count().reset_index()
    u1.rename(columns={'is_trade':'his_shop_'+ss},inplace=True)
#    u1.columns=['shop_id','his_shop_sum_'+ss]
    data1=pd.merge(data1,u1,on=['shop_id',ss],how='left')


string_hour=['item_id','item_category_list','item_brand_id','item_city_id','user_id','user_gender_id','user_occupation_id','user_age_level','context_page_id','shop_id', 'item_property_list_m1', 'item_property_list_m2','item_property_list_m3','predict_category_property_m1']
for ss in string_shop:
    u1=data_day5.groupby(['hour',ss])['is_trade'].count().reset_index()
    u1.rename(columns={'is_trade':'his_hour_'+ss},inplace=True)
    data1=pd.merge(data1,u1,on=['hour',ss],how='left')


string_item=['item_id','item_category_list','item_brand_id','item_property_list_m1', 'item_property_list_m2','item_property_list_m3']

string_user=['user_id','user_gender_id','user_age_level','user_occupation_id','user_star_level']
####每个商品倍多少用户购买
for ss in string_item:
    for ss1 in string_user:
        u1=data_day5.groupby([ss,ss1])['is_trade'].count().reset_index()
        u1.rename(columns={'is_trade':'his'+ss+ss1},inplace=True)
        data1=pd.merge(data1,u1,on=[ss,ss1],how='left')
...
string_use=['user_id','user_gender_id','user_age_level','user_occupation_id','user_star_level','item_pv_level']
string_itemlev=['item_price_level','item_sales_level','item_collected_level']
for ss in string_use:
    for ss1 in string_itemlev:
        u1=data_day5.groupby([ss,ss1])['is_trade'].count().reset_index()
        u1.rename(columns={'is_trade':'his_'+ss+ss1},inplace=True)
        data1=pd.merge(data1,u1,on=[ss,ss1],how='left')


string_item1=['item_id','item_category_list','item_property_list_m1','item_brand_id']
string_itemlev=['item_price_level','item_sales_level','item_collected_level','item_pv_level']
for ss in string_item1:
    for ss1 in string_itemlev:
        u1=data_day5.groupby([ss,ss1])['is_trade'].count().reset_index()
        u1.rename(columns={'is_trade':'his_'+ss+ss1},inplace=True)
        data1=pd.merge(data1,u1,on=[ss,ss1],how='left')
...
for ss in ['item_id','item_category_list','item_property_list_m1', 'item_property_list_m2','item_property_list_m3','item_brand_id','shop_id','shop_id']:
    u1=data_day5.groupby([ss])['is_trade'].sum().reset_index()
    u1.rename(columns={'is_trade':'his_trade_'+ss},inplace=True)
    data1=pd.merge(data1,u1,on=[ss],how='left')
...
before_time_delta_user=pd.merge(before_time_delta_user,time_delta_user,on=['user_id','context_timestamp'],how='left')

before_time_delta_user['is_fisrt_click']=0
before_time_delta_user.loc[(before_time_delta_user.time_delta_user<0),'is_fisrt_click']=1
before_time_delta_user['is_last_click']=0
before_time_delta_user.loc[(before_time_delta_user.time_delta_user>0)&(before_time_delta_user.before_time_delta_user>0),'is_last_click']=1
...
new_train_data=pd.merge(new_train_data,before_time_delta_user_item,on=['user_id','item_id','context_timestamp'],how='left')
del before_time_delta_user_item
new_train_data=pd.merge(new_train_data,before_time_delta_user_brand,on=['user_id','item_brand_id','context_timestamp'],how='left')
del before_time_delta_user_brand
new_train_data=pd.merge(new_train_data,before_time_delta_user_shop,on=['user_id','shop_id','context_timestamp'],how='left')
...
#######今天特征的提取
####id类特征的统计
for ss in ['hour','item_id','item_category_list','item_property_list_m1', 'item_property_list_m2','item_property_list_m3','item_brand_id','item_sales_level','item_price_level',
           'item_collected_level','item_pv_level','user_id','user_gender_id','user_age_level','user_occupation_id','user_star_level','shop_id','shop_review_num_level','shop_star_level']:
    tmp=data.groupby([ss])['is_trade'].count().reset_index()
    tmp.rename(columns={'is_trade':'now_sum_'+ss},inplace=True)
    new_train_data=pd.merge(new_train_data,tmp,on=[ss],how='left')
    


string_hour=['item_id','item_category_list','item_brand_id','item_city_id','user_id','user_gender_id','user_occupation_id','user_age_level','context_page_id','shop_id', 'item_property_list_m1', 'item_property_list_m2','item_property_list_m3','predict_category_property_m1']
for ss in string_shop:
    u1=data.groupby(['hour',ss])['is_trade'].count().reset_index()
    u1.rename(columns={'is_trade':'now_hour_'+ss},inplace=True)
    new_train_data=pd.merge(new_train_data,u1,on=['hour',ss],how='left')

    
new_train_data.item_avescore=0.5*new_train_data.item_sales_level+0.3*new_train_data.item_price_level+0.1*new_train_data.item_collected_level+0.1*new_train_data.item_pv_level
...
trainxy=tr_data[tr_data.hour<10]
valxy=tr_data[tr_data.hour>=10]
...
#sns.countplot(y='hour',hue='is_trade',data=yuce_data)
...
#bst1=lgb.train(params,lgb_train,num_boost_round,feature_name=tr_string,categorical_feature=string_drop)0.163641


pre_off=bst.predict(X_test)
...
######用eval验证
#bst1=lgb.train(params,lgb_train,valid_sets=lgb_eval,num_boost_round=10000,feature_name=tr_string,categorical_feature=string_drop,early_stopping_rounds=100)
bst1=lgb.train(params,lgb_train,valid_sets=lgb_eval,num_boost_round=10000,early_stopping_rounds=100)

pre_off=bst1.predict(X_test)
log_loss(y_test,pre_off)

importance1=bst1.feature_importance()
names=bst1.feature_name()
feat_imp1 = pd.Series(importance1,names).sort_values(ascending=False)
print(feat_imp1)
...
#    lgb_train1=lgb.Dataset(X_train[get_ans_face],y_train,free_raw_data=False)
#    lgb_eval1=lgb.Dataset(X_test[get_ans_face],y_test,free_raw_data=False)
#    bss=lgb.train(params,lgb_train1,valid_sets=lgb_eval1,num_boost_round=10000,early_stopping_rounds=100,verbose_eval=False)
#    pre_on1=bss.predict(online_test)
#    
#
#    return pre_on1
#
#get_ans_face=X_train.columns.values.tolist()
#now_feature = []
#check = 0
...
import pandas as pd
data_ee=pd.read_csv('round2_result/lgb_0163551004657.txt',delimiter=' ',header=0)
asd=data_ee.predicted_score
...
def minux(x):
    if x>0.012:
        ...
        return x
asd=asd.apply(lambda x:minux(x))

data_test=pd.read_csv('D:/mother/round2_ijcai_18_test_a_20180425/round2_ijcai_18_test_b_20180510.txt',delimiter=' ',header=0)

asd=asd.apply(lambda x:float("%.6f" % x))

bao_cun=data_test[['instance_id']]
bao_cun['predicted_score']=asd
bao_cun.to_csv('round2_result/lgb_003603.txt',index=False,sep=' ')
